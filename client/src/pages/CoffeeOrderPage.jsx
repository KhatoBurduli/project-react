...
const CoffeeOrder = () => {
  const { response: responseCoffee, loading: loadingCoffee } = useFetch({ url: "/api/v1/coffees", method: "GET" });
  const { cart, addToCart, removeFromCart } = useCart();
  const navigate = useNavigate();
console.log(cart)
  const coffeeList = responseCoffee?.items.map((item) => ({
    item: item.newCoffee,
    price: item.newPrice,
    id: item._uuid,
  })) || [];

  if (loadingCoffee) return <Spinner/>  
